...
class SendSMSFunctions {
	...
	public function clear_phone_number( $phone_number ) {
		$phone_number = str_replace( array( '+', '-' ), '', filter_var( $phone_number, FILTER_SANITIZE_NUMBER_INT ) );
	 ...
	 * @since 1.0.0
	 */
	public function get_all_phones_subscriber_db() {
		$table_name = $this->wpdb->prefix . 'sendsms_dashboard_subscribers';
		$results    = $this->wpdb->get_results( 'SELECT phone FROM ' . $table_name, ARRAY_A );
		return $results;
	}
	...
	function have_2fa_activated( $userID ) {
		// TODO check if it has a phone number in the first place
